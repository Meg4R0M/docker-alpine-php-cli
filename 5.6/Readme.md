...
Some useful bash functions are also added. For more information see [https://github.com/psykoterro/docker-alpine-php-cli/blob/master/5.6/conf/.bashrc_append](https://github.com/psykoterro/docker-alpine-php-cli/blob/master/5.6/conf/.bashrc_append) 
...
You can run the container with the docker run command :
   ...
   ``` sh
        docker run meg4r0m/alpine-php-cli:5.6
   ```
...
Example : 
   ...
   ``` sh
        docker run --user=$(id -u):$(id -g) meg4r0m/alpine-php-cli:5.6
   ```
...
and then define the SSH_AUTH_SOCK environment variable inside the container with the value of the destination path :     
   ...
   ``` sh
       docker run --user=$(id -u):$(id -g) -v "${SSH_AUTH_SOCK}:/ssh-agent:ro" -e "SSH_AUTH_SOCK=/ssh-agent" meg4r0m/alpine-php-cli:5.6
   ```
